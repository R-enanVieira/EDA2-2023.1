...
static void merge(item *v, int l, int m, int r) {
  ...
  while (i < m && j < r) {
    if (v[i].tam > v[j].tam)
      aux[k++] = v[i++];
    else if (v[i].tam == v[j].tam && v[i].vIncial == J)
      aux[k++] = v[i++];
    else if (v[i].tam == v[j].tam && v[j].vIncial == J)
      aux[k++] = v[j++];
    else if (v[i].tam == v[j].tam && v[i].vIncial < v[j].vIncial)
      aux[k++] = v[i++];
    else
      aux[k++] = v[j++];
  }
...
int main() {
  ...
  if (ans[0].vIncial == J && ans[0].tam == 1)
    printf("Fique em casa\n");
  else if (ans[0].vIncial == J)
